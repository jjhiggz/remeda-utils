...
export function truncate(
  ...
  if (isObject(options)) {
    separator = (("separator" in (options as any)) as any)
      ? (options as any).separator
      : separator;
    length =
      "length" in (options as any)
        ? ((options as any).length as number)
        : (length as number);
    omission =
      "omission" in (options as any)
        ? baseToString((options as any).omission)
        : omission;
  }
